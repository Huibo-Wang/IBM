#include "comDirectForcing.H"

// Constructors

    Foam::comDirectForcing::comDirectForcing
    (
        const fvMesh& mesh,
    ...
    liftDir_(dict.subDict("force").lookupOrDefault("liftDir", vector(0,1,0)))
    {
        ...
        enthalpyLagrange_ = scalarList(number_, 0.0);
...
// ----------------- Properties calculation ------------------ //
    ...
    Foam::scalar Foam::comDirectForcing::getFluMass()
    {
        scalar Pi = constant::mathematical::pi;
        scalar mass;
    ...
    Foam::scalar Foam::comDirectForcing::getParMass()
    {
        scalar Pi = constant::mathematical::pi;
        scalar mass;
    ...
    Foam::scalar Foam::comDirectForcing::getMass()
    {
        return getParMass()-getFluMass();
    }

    Foam::scalar Foam::comDirectForcing::getFluMomentInertia()
    {
        scalar Pi = constant::mathematical::pi;
    ...
    Foam::scalar Foam::comDirectForcing::getParMomentInertia()
    {
        scalar Pi = constant::mathematical::pi;
        scalar momentInertia;
    ...
    Foam::scalar Foam::comDirectForcing::getMomentInertia()
    {
        return getParMomentInertia()-getFluMomentInertia();
    }


    Foam::tensor Foam::comDirectForcing::getJ()
    {
    ...
    void Foam::comDirectForcing::upCoordinates(const vector& displacement)
    {
        vector& center = getCenter();
        center += displacement;
...
// ----------------- IBM direct forcing implementation ------------------ //
    ...
    Foam::scalar Foam::comDirectForcing::deltaFunction(Foam::scalar r)
    {
    ...
    //- find ibCell list
    void Foam::comDirectForcing::ibCellListSet()
    {
        const fvMesh& mesh = getMesh();
    ...
    void Foam::comDirectForcing::calcA()
    {
        const fvMesh& mesh = getMesh();
        const vectorField& C = mesh.cellCentres();
    ...
    Foam::RectangularMatrix<fieldType> Foam::comDirectForcing::parameterCorrection
    (
    ...
    void Foam::comDirectForcing::EulerToLagrange
    (
    ...
    template<class EulerField, class LagrangeField>
    void Foam::comDirectForcing::LagrangeToEuler
    (
    ...
    void Foam::comDirectForcing::solve
    (
        volVectorField& U,
        volScalarField& p,
        volScalarField& h,
    ...
    )
    {
        ...
        const fvMesh& mesh = getMesh();
        const vectorField& C = mesh.cellCentres();
        // ----------------  1. Velocity correction filed calculation ---------------- //
        forAll(coordinates_, i)
        {
            velocityLagrange_[i] = rhof_ * evalPointVelocity(coordinates_[i]);
        }
        ...
        //- Get dU and drhoU on boundary points
        RectangularMatrix<vector> Xu_B = parameterCorrection(rhoU, velocityLagrange_);

        //- Get dU and drhoU on Euler points
        ...
        Info << "the max drhoU is: " << max(drhoU) << endl;

        // ----------------  2. Density correction  ---------------- //
        surfaceScalarField phidrhoU(fvc::interpolate(drhoU) & mesh.Sf());

        fvScalarMatrix drhoEqn
        (
            fvm::ddt(drho) + fvc::div(phidrhoU)
        );

        drhoEqn.solve();

        Info << "the max drho is: " << max(drho) << endl;
        // ----------------  3. Enthalpy and velocity correction  ---------------- //
        // fixed temperature
        forAll(coordinates_, i)
        {
            enthalpyLagrange_[i] = rhof_ * enthalpy_;
        }

        Info << "the max h is: " << max(h) << endl;

        volScalarField rhoh = rho*h;
        Info << "the max rhoh is: " <<  max(rhoh) << endl;

        //- Get drhoh on boundary points
        scalarRectangularMatrix Xh_B = parameterCorrection(rhoh, enthalpyLagrange_);

        //- Get drhoh on Euler points
        scalarRectangularMatrix Xh = A2 * Xh_B;

        Info << "the max drhoh is: " << max(Xh) << endl;
        //- Correct U and h
        forAll(ibCellList_, i)
        {
            label cellI = ibCellList_[i];
            rho[cellI] += drho[i];
            U[cellI] += Xu[i][0]/rho[cellI];
            h[cellI] += Xh[i][0]/rho[cellI];
        }
        ...
        forAll(coordinates_, i)
        {
            ...
            force_ -= Xu_B[i][0] / deltaT ;
            torque_ -= r ^ Xu_B[i][0] / deltaT;
        }
        ...
        coeffs[1] = (force_ & liftDir_)/(ARef_*pDyn);   //lift coefficien

    }

    void Foam::comDirectForcing::update
    (
    ...
    Foam::scalar Foam::comDirectForcing::Ix(const vector& x, const scalar& h)
    {
        scalar Ix, psix, Hs;
    ...
    void Foam::comDirectForcing::clearOut()
    {
        force_ = vector::zero;
        torque_ = vector::zero;
...
//    void Foam::comDirectForcing::velocityNextTimeStep(const Foam::scalar& deltaT, const Foam::vector& g, const Foam::volVectorField& U)
//    {
//        //old part
...
//    void Foam::comDirectForcing::testVelocityNextTimeStep
//    (
...
//    void Foam::comDirectForcing::displacementNextTimeStep(const Foam::scalar& deltaT)
//    {
...
//    void Foam::comDirectForcing::omegaNextTimeStep(const Foam::scalar& deltaT, const Foam::volVectorField& U)
//    {
//        //old part
...
//    void Foam::comDirectForcing::testOmegaNextTimeStep
//    (
//        const volVectorField& U,
//        const volVectorField& Uold,
//        const scalar& deltaT,
...
//    void Foam::comDirectForcing::initialization()
//    {
//        if(motion_)
//        {
//            clearOut();
...
//    void Foam::comDirectForcing::calculateForce
//    (
...
//    void Foam::comDirectForcing::computeCyclicHits(const scalar L)
//    {
//        labelList patches;
//        labelList walls;
...
//    bool Foam::comDirectForcing::onMesh()
//    {
//        label nPtsOnMesh = 0;
//        forAll(coordinates_, pti)
//        {
...
//    Foam::label Foam::comDirectForcing::patchHit
//    (
//        const label patchi,
//        vector &R
//    )
